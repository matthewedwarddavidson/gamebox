// Daily Battleships puzzle: deterministic seed + difficulty from a UTC date.
...
export function dailyFor(date: Date = new Date()): DailyDescriptor {
  ...
  const seed = hashString(`battleships-daily-${dateKey}`);
  const dIndex = hashString(`battleships-daily-diff-${dateKey}`) % DIFFICULTIES.length;
