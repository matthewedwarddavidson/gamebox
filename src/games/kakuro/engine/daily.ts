// Daily Kakuro puzzle: deterministic seed + difficulty from a UTC date.
...
export function dailyFor(date: Date = new Date()): DailyDescriptor {
  ...
  const seed = hashString(`kakuro-daily-${dateKey}`);
  const dIndex = hashString(`kakuro-daily-diff-${dateKey}`) % DIFFICULTIES.length;
