...
describe('emptyStats', () => {
  it('has zeroed totals and all difficulty buckets', () => {
    ...
    expect(Object.keys(s.byDifficulty).sort()).toEqual(['easy', 'hard', 'medium'].sort());
  });
});
...
describe('computeStats', () => {
  ...
  it('counts played and won', () => {
    const s = computeStats([
      game({ status: 'won', difficulty: 'easy', durationMs: 1000 }),
  ...
  it('computes best/avg time', () => {
    const s = computeStats([
      game({ difficulty: 'medium', durationMs: 2000 }),
      game({ difficulty: 'medium', durationMs: 4000 }),
    ]);
