...
export function Home() {
  const startFree = useTrainTracks((s) => s.startFree);
  const startDaily = useTrainTracks((s) => s.startDaily);
  const viewSolution = useTrainTracks((s) => s.viewSolution);
  const stats = useTrainTracks((s) => s.stats);
  const games = useTrainTracks((s) => s.games);
  ...
  return (
    <div className="home">
      <header className="home__header">
        <h1 className="home__title">Train Tracks</h1>
        <p className="home__subtitle">Lay the single track from the row and column counts.</p>
      </header>
